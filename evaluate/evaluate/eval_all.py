# ===================================================================================
# Description: Evaluation of different filters, 
# calculation of position error and orientation error, 
# and real-time visualization of error curves.
...
# Load the data from the CSV file
        ...
        with open("/home/user/ros2_ws/src/evaluate/data/ekf_results.csv") as file:
            reader = csv.reader(file)   
            next(reader) # Skip the header
            for row in reader:
               self.timestamp.append(int(row[0]))
               self.position_error.append(float(row[1]))
               self.orientation_error.append(float(row[2]))
    ...
    def timer_callback(self):                                     # the function for eval 
        ...
        if (self.idx_c0 == 1) and (self.idx_c1 == 1):
            ...
            if eval_posi >= 0.001:
                ...
                self.hx_avg_posi.append(self.avg_posi)
                self.hx_avg_orien.append(self.avg_orien)
                self.num.append(self.i)



                plt.cla()
                plt.plot(self.timestamp, self.position_error, label="Position error - EKF")
                plt.plot(self.timestamp, self.orientation_error, label="Orientation error - EKF")

                plt.plot(self.num, self.hx_avg_posi, label="Position Error - UKF")
                plt.plot(self.num, self.hx_avg_orien, label="Orientation Error - UKF")
                plt.xlabel("Time (s)")
                plt.ylabel("Error (m) (rad)")
                plt.title("The error of the filter")
                # plt.axis("equal")
                plt.legend()
                plt.grid(True)
                plt.pause(0.01)
